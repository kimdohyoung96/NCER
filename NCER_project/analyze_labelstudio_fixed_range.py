...
Usage
-----
python analyze_labelstudio_fixed_range.py --input labelstudio_export.json --outdir ./analysis_out
python analyze_labelstudio_fixed_range.py --input labelstudio_export.json --outdir ./analysis_out --skip-count-check
...
range_split_summary.csv
quality_by_intent.csv
...
def parse_task(item: Dict[str, Any]) -> TaskMetrics:
    ...
    # Rework definition: quality/review event based only
    # 최초 submit 이후 품질 검수/후속 리뷰가 발생한 경우만 재작업으로 간주
    rework_signals = []
    if triage is not None:
        rework_signals.append("triage")
    if answer_quality is not None:
        rework_signals.append("answer_quality")
    if has_doc_comment:
        rework_signals.append("doc_comment")
    if has_answer_comment:
        rework_signals.append("answer_comment")

    is_reworked = len(rework_signals) > 0
    rework_reason = "|".join(rework_signals) if rework_signals else None
...
def build_summary(tasks: List[TaskMetrics]) -> Dict[str, Any]:
    ...
    return {
        ...
        "rework_metrics": {
            "rework_definition": "Quality/review event based only: triage or answer_quality or doc_comment or answer_comment",
